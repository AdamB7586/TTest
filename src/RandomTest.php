...
UNION (SELECT `prim` FROM `".$this->questionsTable."` WHERE `casestudyno` = '".rand(1, 28)."');");
        ...
        return self::$db->insert($this->progressTable, array('user_id' => self::$user->getUserID(), 'questions' => serialize($this->questions), 'answers' => serialize(array()), 'test_id' => $testNo, 'started' => date('Y-m-d H:i:s'), 'status' => 0, 'type' => strtolower($this->getTestType())));
    }
}
