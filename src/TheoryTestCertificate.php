...
use TheoryTest\Car\FPDFProtection;
...
class TheoryTestCertificate implements CertificateInterface
{
     ...
     * @param int|false $alias To view a certificate for someone other than the user logged in set this to the users ID
     */
    public function __construct($user, $theoryTest, $alias = false)
    {
        $this->user = $user;
        $this->theory = $theoryTest;
        $this->pdf = new FPDFProtection();
        if (method_exists($this->user, 'getFirstname') && method_exists($this->user, 'getLastname')) {
            $this->certUsername = $this->user->getFirstname($alias).' '.$this->user->getLastname($alias);
        } elseif (method_exists($this->user, 'getUsername')) {
            $this->certUsername = $this->user->getUsername($alias);
        }
    }
    ...
    public function generateCertificate()
    {
        ...
        $this->pdf->basicTable(
            ...
            [
                array($this->certUsername,
                strip_tags($this->theory->getTestName()),
        ...
        $this->pdf->SetFont('Arial', '', 12);
        if ($this->theory->testresults['status'] == 'pass') {
            $this->pdf->Cell(184, 10, "Congratulations ".trim($this->certUsername));
            $this->pdf->Ln(4);
            ...
            $this->pdf->Cell(184, 10, "Sorry ".trim($this->certUsername).", but you have not passed this time.");
            $this->pdf->Ln(4);
    ...
    protected function overallResults()
    {
        ...
        foreach ($this->theory->getCategories() as $data) {
            ...
            $groupdata[] = [
                $data['section'],
                substr($data['name'], 0, 53),
                $correct,
                $incorrect,
                $total,
                number_format(intval(($correct / $total) * 100), 0).'%',
                ''
            ];
        ...
        foreach ($this->theory->questions as $question => $prim) {
            if (isset($this->theory->useranswers[$question]['status']) && $this->theory->useranswers[$question]['status'] == '4') {
                $correct = 'Correct';
            } else {
                $correct = 'Incorrect';
            }
