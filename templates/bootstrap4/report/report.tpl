    ...
    <div class="col-lg-12">
        <table class="table table-striped table-bordered table-hover table-sm">
            ...
            <tr>
                ...
                {if isset($test.status) && $test.status >= 1}<td class="text-center bg-{if $result|strtolower == 'failed'}danger{else}success{/if}">{$result}</td><td class="text-center"><a href="/certificate.pdf?testID={$i}" title="View PDF" target="_blank"><span class="fas fa-file-pdf-o"></span> View PDF {if $test.status == 1}Certificate{else}Report{/if}</a></td>
